...
export default function ProductDetailPage() {
  ...
  // Carico il prodotto in base all'ID
  useEffect(() => {
    if (mounted && params.id) {
      ...
      if (foundProduct) {
        ...
        // Trova prodotti correlati della stessa categoria
        const related = products
          .filter((p) => p.category === foundProduct.category && p.id !== foundProduct.id)
          .slice(0, 3)
        setRelatedProducts(related)
      } else {
        // Prodotto non trovato, reindirizza alla pagina dei prodotti
        router.push("/shop/products")
      }
    }
  }, [params.id, mounted, router])
